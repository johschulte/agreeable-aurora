...
export const onRequest = async (context, next) => {
  ...
  if (context.request.method === 'POST') {
    ...
    try {
      ...
      console.log('Middleware: Formularfelder:', JSON.stringify(formEntries, null, 2));
      
      // Setze einige nützliche Diagnostikdaten in locals
      context.locals.formDataParsed = true;
      context.locals.formEntries = formEntries;
    } catch (error) {
      console.error('Middleware: Fehler beim Parsen der FormData:', error.message);
      ...
      try {
        ...
        if (contentType?.includes('application/x-www-form-urlencoded')) {
          try {
            ...
            console.log('Middleware: Manuell geparste Formularfelder:', JSON.stringify(manualEntries, null, 2));
            context.locals.manualFormEntries = manualEntries;
          } catch (e) {
            console.error('Middleware: Konnte Daten nicht manuell parsen:', e.message);
          }
        }
      } catch (textError) {
        console.error('Middleware: Konnte Rohtext nicht lesen:', textError.message);
      }
    }
  }
